...
import com.wix.vivaLaVita.config.UserConfig
...
class DBQueries[F[_]: Sync](profile: JdbcProfile, schema: Schema, admin: UserConfig, transform: DBIO ~> F)(implicit executionContext: ExecutionContext) extends Queries[F] {
  ...
  def setup: F[Unit] = for {
    ...
    //
    _ <- userDao.select(admin.email).flatMap(usr => if(usr.isDefined) {
      Sync[F].pure(())
    } else {
      BCrypt.hashpw[F](admin.password).flatMap(psw => userDao.create(UserDTO.buildUser(UserRequest(admin.name, admin.email, Some(psw))))).map(_ => ())
    })
  } yield ()
}
